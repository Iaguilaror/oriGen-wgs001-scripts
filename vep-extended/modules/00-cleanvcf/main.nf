...
process cleanvcf {
    ...
    """
    bcftools annotate \
      -x ID,QUAL,FILTER,FORMAT,^INFO/AC,INFO/AN,INFO/AF \
      $vcf \
    | bcftools filter \
      -i 'AC > 2' \
      - \
    ...
    | bcftools annotate \
      -x FILTER \
      -c INFO/AF_ori:=INFO/AF,INFO/AC_ori:=INFO/AC,INFO/AN_ori:=INFO/AN \
      - \
    | cut -f1-8 \
    | bgzip --threads 4 > ${vcf.simpleName}_clean.vcf.gz \
